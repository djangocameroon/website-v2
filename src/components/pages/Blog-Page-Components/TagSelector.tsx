      ...
      <div className="flex flex-wrap urbanist-font gap-2 mb-4">
        {availableTags.map((tag) => (
          <button
            ...
            className={`px-4 py-2 text-xs uppercase tracking-widest rounded-lg transition-all border active:scale-105 ${
              selectedTags.includes(tag)
                ? 'bg-blue-600 text-white font-bold'
                : 'bg-[#D9E7FF] text-blue-600 hover:bg-blue-100 border-secondary'
            }`}
          >
            {tag}
          </button>
      ...
      {selectedTags.length > 0 && (
        <div className="flex flex-wrap gap-2 p-4 urbanist-font bg-gray-50 rounded-xl">
          {selectedTags.map((tag) => (
            <span
              key={tag}
              className="px-3 py-1.5 bg-blue-600 text-white font-bold text-xs uppercase tracking-widest rounded-lg flex items-center gap-2"
            >
              {tag}
              <button
                type="button"
